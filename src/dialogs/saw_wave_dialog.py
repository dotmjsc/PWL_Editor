"""
Sawtooth waveform dialog for the PWL Editor.
...
class SawWaveGeneratorDialog:
    """Modal dialog that inserts a synthesized saw waveform into the editor."""
    ...
    def __init__(self, editor: Any, parent: Optional[tk.Tk] = None) -> None:
        ...
        self.window.title("Generate Saw Wave")
        ...
        self.ramp_fraction_var = tk.StringVar(value="1")
        ...
        self._preview_signature: Optional[tuple[SawWaveConfig, str]] = None
    ...
    def _apply_last_settings(self) -> None:
        ...
        self.ramp_fraction_var.set(f"{config.ramp_fraction:g}")
    ...
    def _build_ui(self) -> None:
        ...
        form_frame = ttk.LabelFrame(main_frame, text="Saw Wave Parameters")
        ...
        ttk.Label(form_frame, text="Ramp fraction (0-1)").grid(row=3, column=0, sticky="w", padx=8, pady=4)
        ttk.Entry(form_frame, textvariable=self.ramp_fraction_var, width=14).grid(row=3, column=1, sticky="ew", padx=8, pady=4)
        ...
        ttk.Label(form_frame, text="Reset edge (ppm of period)").grid(row=6, column=0, sticky="w", padx=8, pady=4)
    ...
    def _on_preview(self) -> None:
        ...
        self.editor.status_var.set(
            f"Saw wave preview applied (not saved). {summary}"
        )
    ...
    def _on_apply(self) -> None:
        ...
        if warnings:
            summary += " | Warnings: " + "; ".join(warnings)
        self.editor.status_var.set(f"Saw wave generated. {summary}")
        self._close_dialog()
    ...
    def _on_cancel(self) -> None:
        ...
        self.result = None
        self.editor.status_var.set(
            "Saw wave generation cancelled; original data restored"
        )
        self._close_dialog()
    ...
    def _gather_settings(
        ...
        try:
            ...
            ramp_fraction = float(self.ramp_fraction_var.get())
            ...
            if show_errors:
                messagebox.showerror("Generate Saw Wave", f"Invalid parameter: {exc}")
            return None
        ...
        if not 0 < ramp_fraction <= 1:
            if show_errors:
                messagebox.showerror(
                    "Generate Saw Wave",
                    "Ramp fraction must be greater than 0 and at most 1.",
                )
            return None
        if edge_ppm < 0:
            if show_errors:
                messagebox.showerror(
                    "Generate Saw Wave", "Reset edge must be non-negative."
                )
            return None
    ...
    def _generate_preview(
        ...
        config: SawWaveConfig = settings["config"]

        try:
            result = generate_saw_wave(config)
        except SawWaveValidationError as exc:
            messagebox.showerror("Generate Saw Wave", str(exc))
            return None
        ...
        if combined is None:
            messagebox.showinfo(
                "Generate Saw Wave", "Generation produced no points."
            )
            return None
    ...
    def _remember_settings(self, config: SawWaveConfig, apply_mode: str) -> None:
        global _LAST_SETTINGS
...
__all__ = ["SawWaveGeneratorDialog"]
